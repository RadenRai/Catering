...
if (!function_exists('base_url')) {
	...
	else $base_url = 'http://localhost/cateringbaru/';
	if ($parse) {
...
class paket
{
	...
	{
		$ambil = $this->koneksi->query("SELECT * FROM paket WHERE id_paket='$id_paket'");
		$pecahdata = $ambil->fetch_assoc();
		return $pecahdata;
	}
	function tampil()
	{
		$semuadata = array();
		$ambil = $this->koneksi->query("SELECT * FROM paket ORDER BY id_paket");
		while ($pecahdata = $ambil->fetch_assoc())
		{
			$semuadata[] = $pecahdata;
		}
	...
	{
		$semuadata = array();
		$ambil = $this->koneksi->query("SELECT * FROM paket ORDER BY id_paket ASC LIMIT 6");
		while ($pecahdata = $ambil->fetch_assoc())
		{
			$semuadata[] = $pecahdata;
		}
	...
	{
		$ambil = $this->koneksi->query("SELECT * FROM paket WHERE nama_paket='$nama'");
		$yangcocok = $ambil->num_rows;
		if ($yangcocok > 0)
		{
			return "gagal";
		...
		else
		{
			...
			move_uploaded_file($lokasi, "../upload/img-paket/$fix");
			$this->koneksi->query("INSERT INTO paket (nama_paket,uraian,gambar) VALUES ('$nama','$uraian','$fix')")or die(mysqli_error($this->koneksi));
			return "sukses";
		}
	}
	function ubah($nama,$gambar,$uraian,$id_paket)
	{
		$namagambar=$gambar['name'];
		$lokasi=$gambar['tmp_name'];
		$tanggal = date('Ymd');
		...
		if (!empty($lokasi)) {
			$fotolama=$this->detail($id_paket);
			$gambarlama=$fotolama['gambar'];
			if (file_exists("../upload/img-paket/$gambarlama")) {
				unlink("../upload/img-paket/$gambarlama");
			}
			move_uploaded_file($lokasi, "../upload/img-paket/$fix");
			$this->koneksi->query("UPDATE paket SET nama_paket='$nama',gambar='$fix', uraian='$uraian' WHERE id_paket='$id_paket' ")or die(mysqli_error($this->koneksi));
		}
		else
		{
			$this->koneksi->query("UPDATE paket SET nama_paket='$nama', uraian='$uraian' WHERE id_paket='$id_paket' ")or die(mysqli_error($this->koneksi));
		}
	}
	function hapus($id_paket)
	{
		$datapaket=$this->detail($id_paket);
		$gambar=$datapaket['gambar'];
		if (file_exists("../upload/img-paket/$gambar")) {
			unlink("../upload/img-paket/$gambar");
		}
		$this->koneksi->query("DELETE FROM paket WHERE id_paket='$id_paket'");
	}
}
...
class menu
{
	...
	{
		$ambil = $this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket WHERE id_menu='$id_menu'");
		$pecahdata = $ambil->fetch_assoc();
		return $pecahdata;
	}
	...
	{
		$semuadata = array();
		$ambil = $this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket ORDER BY id_menu");
		while ($pecahdata = $ambil->fetch_assoc())
		{
			$semuadata[] = $pecahdata;
		}
	...
	// menampilkan data dari database
		$ambildata=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket  ORDER BY menu.id_menu DESC LIMIT $posisi,$batas")or die(mysqli_error($this->koneksi));
	...
	function menuterbaru()
	{
		$semuadata = array();
	// menampilkan data dari database
		$ambildata=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket ORDER BY menu.id_menu DESC LIMIT 6")or die(mysqli_error($this->koneksi));
	// memecah array dan diperulangkan
		while($pecahdata=$ambildata->fetch_assoc())
		{
			$semuadata[]=$pecahdata;
	...
	function menuterlaris()
	{
		$semuadata = array();
	// menampilkan data dari database
		$ambildata=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket WHERE terjual!='' ORDER BY terjual DESC LIMIT 8")or die(mysqli_error($this->koneksi));
	// memecah array dan diperulangkan
		while($pecahdata=$ambildata->fetch_assoc())
		{
			$semuadata[]=$pecahdata;
	...
	function tambah($id_paket,$nama,$harga,$deskripsi,$gambar)
	{
		...
		else
		{
			...
			$lokasi = $gambar['tmp_name'];
			move_uploaded_file($lokasi, "../upload/img-menu/$fix");
			$this->koneksi->query("INSERT INTO menu (id_paket,nama_menu,harga_menu,deskripsi_menu,gambar_menu) VALUES ('$id_paket','$nama','$harga','$deskripsi','$fix')")or die(mysqli_error($this->koneksi));
			return "sukses";
		}
	}
	function ubah($id_paket,$nama,$harga,$deskripsi,$gambar,$id_menu)
	{
		$namagambar=$gambar['name'];
		$lokasi=$gambar['tmp_name'];
		$tanggal = date('Ymd');
		...
		if (!empty($lokasi)) {
			...
			move_uploaded_file($lokasi, "../upload/img-menu/$fix");
			$this->koneksi->query("UPDATE menu SET id_paket='$id_paket',  nama_menu='$nama',
	...
	function paketmenu($id_paket,$posisi,$batas)
	{
		$semuadata=array();
		$ambildata=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket WHERE menu.id_paket='$id_paket' ORDER BY id_menu DESC LIMIT $posisi,$batas ");
	...
	function semuamenu($id_paket)
	{
		$data=$this->koneksi->query("SELECT * FROM menu WHERE id_paket='$id_paket'");
		$total=$data->num_rows;
		return $total;
	}
	function carimenu($keyword,$posisi,$batas)
	{
		...
		$data=array();
		$ambil=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket  WHERE nama_menu LIKE '%$keyword%' OR deskripsi_menu LIKE '$$keyword' ORDER BY id_menu DESC LIMIT $posisi,$batas");
	...
	function semuamenucari($keyword){
		$data=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket  WHERE nama_menu LIKE '%$keyword%' OR deskripsi_menu LIKE '$$keyword'");
		$total=$data->num_rows;
		return $total;
	}
}
...
class pelanggan
{
	...
	function daftar($nama,$username,$email,$password,$telepon,$alamat)
	{
		...
		else
		{
			...
			$to       = $email;
			$subject  = 'Info akun Marwah Catering';
			$message  = '
			<!doctype html>
			<html>
			<head>
			<meta charset="utf-8">
			<title>Info akun Marwah Catering</title>
			...
			<h3 align="center">Info akun Marwah Catering</h3>
			<table width="100%">
			<tr>
			...
			<tr>
			<td><center><a style="color: #fff;" href="http://localhost/cateringbaru/aktivasi?t='.$token.'" class="aktivasi">Aktivasi Akun</a></center></td>
			</tr>
			<tr>
			<td><p>Atau kamu dapat menyalin link berikut untuk aktivasi akun anda <br> <a href="http://localhost/cateringbaru/aktivasi?t='.$token.'">http://localhost/cateringbaru/aktivasi?t='.$token.'</a></p></td>
			</tr>
			<hr>
			<tr>
	...
	function lupa_password($email)
	{
		...
		else
		{
			...
			<tr>
			<td class="title" >
			<h5 style="margin-top: -7px;">Marwah Catering</h5>
			</td>

			<td class="invoice">
			Telp : +628981189598 <br>
			Email : marwahcatringinfo@gmail.com<br>
			Alamat : Nogosaren, Nogotirto, <br>Kec. Gamping, Kabupaten Sleman, <br>Daerah Istimewa Yogyakarta 55293
			</td>
			</tr>
			</table>
			</td>
...
class pesanan
{
	...
	{
			// menampilkan dari tabel pembayan yang id pelanggannyaa ($id_pelanggan)
	...
	function checkout($nama,$telp,$alamat,$paket,$tanggal_aa,$jam_aa,$biaya_paket,$total_belanja)
	{
		...
		$total_pesanan = $biaya_paket+$total_belanja;
		$dplunas = $total_pesanan/2;
		$this->koneksi->query("INSERT INTO pesanan (id_pelanggan,id_ongkirket,tanggal_pesanan,deadline_dp,deadline_lunas,tanggal_pengiriman,jam_pengiriman,status_pesanan,status_pembayaran,dp,lunas,total_belanja,total_ongkir,total_pesanan,nama_penerima,telepon_penerima,alamat_penerima) VALUES ('$id_pelanggan','$paket','$tanggal_pesan','$deadlinedp','$deadlinelunas','$tanggalaa','$jam_aa','$status','$status_p','$dplunas','$dplunas','$total_belanja','$biaya_paket','$total_pesanan','$nama','$telp','$alamat')")or die(mysqli_error($this->koneksi));
	...
	function kirim_pembayaran($nama,$bank,$jumlah,$id_pesanan,$id_pelanggan)
	{
	...
	function kirim_pembayaran_pelunasan($nama,$bank,$tanggal,$jumlah,$id_pesanan,$id_pelanggan)
	{
		$tglkon=date("Y-m-d H:i:s");
		$keperluan = "Pelunasan";
		$this->koneksi->query("INSERT INTO pembayaran (id_pesanan,id_pelanggan,tanggal_konfirmasi,tanggal_bayar,atas_nama,bank,jumlah,keperluan)
...
class laporan
{
	...
	{
		$semuadata=array();
		$ambil=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket WHERE terjual!=0 ORDER BY terjual DESC LIMIT 10");
		while ($pecah=$ambil->fetch_assoc()) {
			$semuadata[]=$pecah;
		}
		return $semuadata;
	...
	{
		$semuadata=array();
		$ambil=$this->koneksi->query("SELECT * FROM menu JOIN paket ON menu.id_paket=paket.id_paket WHERE terjual=0 ORDER BY terjual ASC LIMIT 5");
		while ($pecah=$ambil->fetch_assoc()) {
			$semuadata[]=$pecah;
		}
		return $semuadata;
...
class ongkir
{
	...
	function simpan($kota,$ongkir,$id_ongkirket)
	{
		$ambil = $this->koneksi->query("SELECT * FROM ongkir WHERE kabupaten='$kota'");
		...
		{
			$this->koneksi->query("INSERT INTO ongkir (id_ongkirket,kabupaten,ongkos_kirim) VALUES('$id_ongkirket', '$kota', '$ongkir') ");
			return "sukses";
		}
	...
	{
		$semuadata = array();
		$ambil=$this->koneksi->query("SELECT * FROM ongkirket JOIN ongkir ON ongkirket.id_ongkirket=ongkir.id_ongkirket WHERE ongkir.id_ongkirket='$id_ongkirket' ");
		while ($pecahdata=$ambil->fetch_assoc()) 
		{
			$semuadata[]=$pecahdata;
		}
	...
	function apiubah($id_ongkir)
	{
		$ambil=$this->koneksi->query("SELECT * FROM ongkir WHERE id_ongkir='$id_ongkir' ");
		$pecah=$ambil->fetch_assoc();
		return $pecah;
	}
	function ubah($id_ongkir,$kabupaten,$ongkos_kirim)
	{
		$this->koneksi->query("UPDATE ongkir SET kabupaten='$kabupaten',ongkos_kirim='$ongkos_kirim' WHERE id_ongkir='$id_ongkir'");
	}
	function hapus($id_ongkir)
	{
